				...
				<div class="panel panel-flat  col-lg-6">
					...
					<div class="panel-body">
						<table class="table table-striped table-bordered sp-a-40"
							...
							<thead>
								<tr>
									<th>Client Code Types</th>
									<th>Digits</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${clientCode}" var="c">
									<tr>
										<td>${c.clientcodeType}</td>
										<td>
										<c:if test="${c.clientcodeDigits == 0}">
											auto
										</c:if> 
										<c:if test="${c.clientcodeDigits != 0}">
											${c.clientcodeDigits} 
										</c:if></td>
										<td><form:form servletRelativeAction="/editClientCode"
												method="POST">
												<input type="hidden" name="clientcodeId" value="${c.clientcodeId}" />
												<button type="submit" class="btn btn-primary btn-xs"
													style="padding-left: 12px; padding-right: 13px;">
													Edit <span class="glyphicon glyphicon-edit"></span>
												</button>
											</form:form>&nbsp; <a href="deleteClientCode?clientcodeId=${c.clientcodeId}"
											title="Delete"
